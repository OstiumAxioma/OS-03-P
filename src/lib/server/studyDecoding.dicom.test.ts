...
const SERIES_UID = "1.2.826.0.1.3680043.10.543.100";
...
function dicomSlice(instance: number): Buffer {
  ...
  [-100 + instance * 20, 0 + instance * 20, 100 + instance * 20, 200 + instance * 20].forEach((value, index) => {
  ...
  const dataset = Buffer.concat([
    ...
    element(0x0020, 0x000e, "UI", padded(SERIES_UID, 0)),
...
describe("decodeDicomFiles", () => {
  it("decodes a real two-slice DICOM series through ITK-Wasm", async () => {
    await withTemporaryDirectory(async (directory) => {
      ...
      expect(payload.selectedSeriesUid).toBe(SERIES_UID);
